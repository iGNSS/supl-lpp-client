...
static void gen_msm_sat(rtcm_t *rtcm, int sys, int nsat,
                        const unsigned char *sat_ind, OptDouble *rrng, OptDouble *rrng_mod,
                        OptDouble *rrate, unsigned char *info)
{
    obsd_t *data;
    double lambda;
    int i,j,k,sat,sig,f,fcn;
    OptDouble rrng_s;
    OptDouble rrng_mod_s;
    OptDouble rrate_s;
    ...
    for (i=0;i<rtcm->obs.n;i++) {
        ...
        for (j=0;j<NFREQ+NEXOBS;j++) {
            ...
            rrng_s = data->integer_ms[j];
            rrng_mod_s = data->rough_range_modulo_ms[j];
            rrate_s = data->rough_phase_range_rates[j];
...
static void gen_msm_sig(rtcm_t *rtcm, int sys, int nsat, int nsig, int ncell,
                        ...
                        const unsigned char *cell_ind, const OptDouble *rrng,
                        const OptDouble *rrate, OptDouble *psrng, OptDouble *phrng,
                        OptDouble *rate, OptDouble *lock, unsigned char *half,
                        OptDouble *cnr)
{
    obsd_t *data;
    double lambda;
    ...
    for (i=0;i<rtcm->obs.n;i++) {
        ...
        for (j=0;j<NFREQ+NEXOBS;j++) {
            ...
            psrng_s = data->fine_pseudo_range[j];
            phrng_s = data->fine_phase_range[j];
            rate_s = data->fine_phase_range_rates[j];
            lt = data->phase_range_lock[j];
            unsigned char half_s = data->half_cycle[j];
            
            if (psrng) psrng[cell-1]=psrng_s;
            if (phrng) phrng[cell-1]=phrng_s;
            if (rate ) rate [cell-1]=rate_s;
            if (lock) lock[cell-1]=lt;
            if (half) half[cell-1]=half_s;
...
static int encode_msm_head(int type, rtcm_t *rtcm, int sys, int sync, int *nsat,
                           ...
                           unsigned char *info, OptDouble *psrng, OptDouble *phrng,
                           OptDouble *rate, OptDouble *lock, unsigned char *half,
                           OptDouble *cnr)
{
    double tow;
...
static int encode_msm_psrng(rtcm_t *rtcm, int i, const OptDouble *psrng, int ncell)
{
    ...
    for (j=0;j<ncell;j++) {
        ...
        else if (fabs(psrng[j].value)>292.7) {
...
static int encode_msm_phrng(rtcm_t *rtcm, int i, const OptDouble *phrng, int ncell)
{
    ...
    for (j=0;j<ncell;j++) {
        ...
        else if (fabs(phrng[j].value)>1171.0) {
...
static int encode_msm_lock(rtcm_t *rtcm, int i, const OptDouble *lock, int ncell)
{
    ...
    for (j=0;j<ncell;j++) {
        if(!lock[j].available) {
            ...
            lock_val=to_msm_lock(lock[j].value);
        }
                              ...
                              int ncell)
{
    ...
    for (j=0;j<ncell;j++) {
        if(!lock[j].available) {
            ...
            lock_val=to_msm_lock_ex(lock[j].value);
        }
...
static int encode_msm_cnr(rtcm_t *rtcm, int i, const OptDouble *cnr, int ncell)
{
    ...
    for (j=0;j<ncell;j++) {
        if(!cnr[j].available || cnr[j].value <= 0 || cnr[j].value >= 64) {
            cnr_val = 0;
        } else {
            cnr_val=ROUND(cnr[j].value/1.0);
        }
...
static int encode_msm_cnr_ex(rtcm_t *rtcm, int i, const OptDouble *cnr, int ncell)
{
    ...
    for (j=0;j<ncell;j++) {
        if(!cnr[j].available || cnr[j].value <= 0 || cnr[j].value >= 64) {
            ...
            cnr_val=ROUND(cnr[j].value/0.0625);
        }
...
static int encode_msm_rate(rtcm_t *rtcm, int i, const OptDouble *rate, int ncell)
{
    ...
    for (j=0;j<ncell;j++) {
         ...
         else if (fabs(rate[j].value)>1.6384) {
...
static int encode_msm7(rtcm_t *rtcm, int sys, int sync)
{
    ...
    /* encode msm signal data */
    i=encode_msm_psrng_ex(rtcm,i,psrng,ncell); /* fine pseudorange */
    i=encode_msm_phrng_ex(rtcm,i,phrng,ncell); /* fine phase-range */
    i=encode_msm_lock_ex(rtcm,i,lock ,ncell); /* lock-time indicator */
    i=encode_msm_half_amb(rtcm,i,half ,ncell); /* half-cycle-amb indicator */
    i=encode_msm_cnr_ex (rtcm,i,cnr  ,ncell); /* signal cnr */
    i=encode_msm_rate (rtcm,i,rate ,ncell); /* fine phase-range-rate */
    rtcm->nbit=i;
    return 1;
}
