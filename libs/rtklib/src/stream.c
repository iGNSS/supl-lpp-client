...
static serial_t *openserial(const char *path, int mode, char *msg)
{
    ...
    sprintf(msg,"%s",port);
#else
    snprintf(dev,sizeof(dev),"/dev/%s",port);
...
#endif
{
    ...
    snprintf(local,sizeof(local),"%s%c%s",localdir,FILEPATHSEP,p);
    snprintf(errfile,sizeof(errfile),"%s.err",local);
    ...
    if (ftp->proto==0) { /* ftp */
        snprintf(opt,sizeof(opt),"--ftp-user=%s --ftp-password=%s --glob=off --passive-ftp %s-t 1 -T %d -O \"%s\"",
                ftp->user,ftp->passwd,proxyopt,FTP_TIMEOUT,local);
        snprintf(cmd,sizeof(cmd),"%s%s %s \"ftp://%s/%s\" 2> \"%s\"\n",env,FTP_CMD,opt,ftp->addr,
                remote,errfile);
    }
    else { /* http */
        snprintf(opt,sizeof(opt),"%s-t 1 -T %d -O \"%s\"",proxyopt,FTP_TIMEOUT,local);
        snprintf(cmd,sizeof(cmd),"%s%s %s \"http://%s/%s\" 2> \"%s\"\n",env,FTP_CMD,opt,ftp->addr,
                remote,errfile);
    }
