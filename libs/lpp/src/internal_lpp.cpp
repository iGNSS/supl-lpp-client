...
LPP_Message* lpp_decode(OCTET_STRING* data) {
    ...
    asn_dec_rval_t rval = uper_decode_complete(
        0, &asn_DEF_LPP_Message, (void**)&lpp, data->buf, data->size);
    if (rval.code != RC_OK) {
        return NULL;
    }
...
OCTET_STRING* lpp_encode(LPP_Message* lpp) {
    char           buffer[1 << 16];
    asn_enc_rval_t ret = uper_encode_to_buffer(&asn_DEF_LPP_Message, NULL, lpp,
...
LPP_Message* lpp_create(LPP_Transaction*       transaction,
                        LPP_MessageBody__c1_PR pr) {
    LPP_Message* lpp    = ALLOC_ZERO(LPP_Message);
    lpp->endTransaction = 0;

    if (transaction) {
        LPP_TransactionID* tid = ALLOC_ZERO(LPP_TransactionID);
        tid->initiator = transaction->initiator == 0 ? Initiator_targetDevice :
...
OCTET_STRING* lpp_abort(LPP_Transaction* transaction) {
    transaction->end = 1;
    LPP_Message* lpp_abort_msg =
        lpp_create(transaction, LPP_MessageBody__c1_PR_abort);
    LPP_MessageBody_t* body = lpp_abort_msg->lpp_MessageBody;
    body->choice.c1.choice.abort.criticalExtensions.present =
    ...
    Abort_r9_IEs_t* abort_message =
        &body->choice.c1.choice.abort.criticalExtensions.choice.c1.choice
             .abort_r9;
...
int lpp_is_provide_assistance_data(LPP_Message* lpp) {
    ...
    if (!lpp->lpp_MessageBody)
        return 0;
    if (lpp->lpp_MessageBody->present != LPP_MessageBody_PR_c1)
        return 0;
...
long lpp_get_periodic_id(LPP_Message* lpp) {
    ...
    if (c1_pad->criticalExtensions.present !=
    ...
    auto pad =
        &c1_pad->criticalExtensions.choice.c1.choice.provideAssistanceData_r9;
    if (!pad->commonIEsProvideAssistanceData)
        return 0;
    if (!pad->commonIEsProvideAssistanceData->ext2)
        return 0;
    if (!pad->commonIEsProvideAssistanceData->ext2->periodicAssistanceData_r15)
        return 0;

    auto periodic_ad =
        pad->commonIEsProvideAssistanceData->ext2->periodicAssistanceData_r15;
...
int lpp_is_abort(LPP_Message* lpp) {
    ...
    if (lpp->lpp_MessageBody->present != LPP_MessageBody_PR_c1)
        return 0;
    if (lpp->lpp_MessageBody->choice.c1.present != LPP_MessageBody__c1_PR_abort)
        return 0;
    return 1;
}
...
static auto lpp_PLI_CIE_ext2(LocationInformation* li) {
    auto ext2 = ALLOC_ZERO(CommonIEsProvideLocationInformation_t::
                               CommonIEsProvideLocationInformation__ext2);
    ext2->locationSource_r13 = asn_bit_string_create();
    asn_bit_string_set_length(ext2->locationSource_r13, 6);
    asn_bit_string_set(ext2->locationSource_r13,
                       LocationSource_r13_ha_gnss_v1510, 1);
...
lpp_PLI_CommonIEsProvideLocationInformation(LocationInformation* li,
    ...
    if (has_information) {
        ...
        LE->locationfailurecause =
            LocationFailureCause_periodicLocationMeasurementsNotAvailable;
...
OCTET_STRING* lpp_PLI_location_estimate(LPP_Transaction*     transaction,
                                        LocationInformation* li,
                                        bool                 has_information) {
    //
    // Init body
    //
    auto lpp  = lpp_create(transaction,
                          LPP_MessageBody__c1_PR_provideLocationInformation);
    auto body = lpp->lpp_MessageBody;
    body->choice.c1.choice.provideLocationInformation.criticalExtensions
        .present = ProvideLocationInformation__criticalExtensions_PR_c1;
    body->choice.c1.choice.provideLocationInformation.criticalExtensions.choice
        .c1.present =
    ...
    ProvideLocationInformation_r9_IEs_t* PLI =
        &body->choice.c1.choice.provideLocationInformation.criticalExtensions
             .choice.c1.choice.provideLocationInformation_r9;

    auto CIE_PLI =
        lpp_PLI_CommonIEsProvideLocationInformation(li, has_information);
    PLI->commonIEsProvideLocationInformation = CIE_PLI;

    // Finalize and return
    OCTET_STRING_t* result = lpp_encode(lpp);
    lpp_destroy(lpp);
    return result;
}

ECID_ProvideLocationInformation_t*
lpp_PLI_get_ECID_ProvideLocationInformation(ECIDInformation* ecid,
                                            bool             has_information) {
    ...
    CGI->cellIdentity.present =
        CellGlobalIdEUTRA_AndUTRA__cellIdentity_PR_eutra;
    asn_bit_string_set_length(&CGI->cellIdentity.choice.eutra, 28);
    // This solution with a shift might cause problems down the line, but
    // doesn't encode otherwise
    asn_bit_string_set_s32_shifted(&CGI->cellIdentity.choice.eutra,
                                   ecid->cell.cell, 4);
    ...
    temp = mcc - temp * 100;
    asn_sequence_add(&CGI->plmn_Identity.mcc,
                     newLong((int)(temp - temp % 10) / 10));
...
OCTET_STRING* lpp_PLI_location_measurements(LPP_Transaction* transaction,
